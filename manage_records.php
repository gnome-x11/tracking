<!-- eto yung form pag mag veverify kana -->
...
session_start();
if (isset($_GET['id'])) {
	$qry = $conn->query("SELECT t.*,concat(p.lastname,', ',p.firstname,' ',p.middlename) as name, concat(p.address,', ',p.street,', ',p.baranggay,', ',p.city,', ',p.state,', ',p.zip_code) as caddress,e.name as ename,p.student_id FROM person_tracks t inner join persons p on p.id = t.person_id inner join establishments e on e.id = t.establishment_id  where t.id= " . $_GET['id']);
	...
	<form action="" id="manage-records">
		...
		<div class="form-group">
			...
			<input type="number" class="form-control" id="student_id" name="student_id"
				value="<?php echo isset($student_id) ? $student_id : '' ?>" required autocomplete="off">
		</div>
		<div class="row">
			<div class="col-md-12">
				<button class="btn btn-sm btn-primary btn-sm col-sm-2 btn-block float-right" type="button"
...
<script>
	...
	function get_person() {
		...
		$.ajax({
			...
			method: "POST",
			data: { tracking: $('#tracking_id').val() },
			success: function (resp) {
				if (resp) {
					resp = JSON.parse(resp)
					if (resp.status == 1) {
						$('photo').html('src', 'uploads/' + resp.photo)
						$('#name').html(resp.name)
